...
/**
 * Initialize a new Good Partner project
 * @param {string} projectSlug - Directory name for the project
 * @param {string} langCode - 'ko', 'en', 'zh'
 */
export async function initProject(projectSlug, langCode) {
  const root = process.cwd(); // User's current directory where they ran the command
  ...
  const rcPath = path.join(path.join(root, 'good-partner'), '.good-partner-rc.json'); // Global config? Or per project?
  // Let's put a config in the project itself for now, or the root if it's the main repo.
  // Assuming the user runs this in the root of "collaboration hub".
...
/**
 * Install AI Adapters
 * @param {string[]} adapters - ['Codex', 'Claude', 'Gemini']
 */
